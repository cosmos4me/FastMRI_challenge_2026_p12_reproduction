...
from utils.common.loss_function import BBoxAwareSSIMLoss
...
def train_epoch(args, epoch, model, data_loader, optimizer, loss_type):
    ...
    for iter, data in enumerate(data_loader):
        mask, kspace, target, maximum, bbox_mask, foreground_mask, _, _ = data
        mask = mask.cuda(non_blocking=True)
        kspace = kspace.cuda(non_blocking=True)
        target = target.cuda(non_blocking=True)
        maximum = maximum.cuda(non_blocking=True)
        bbox_mask = bbox_mask.cuda(non_blocking=True)
        foreground_mask = foreground_mask.cuda(non_blocking=True)

        output = model(kspace, mask)
        loss = loss_type(output, target, maximum, bbox_mask, foreground_mask)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
...
def validate(args, model, data_loader):
    ...
    with torch.no_grad():
        for iter, data in enumerate(data_loader):
            mask, kspace, target, _, _, _, fnames, slices = data[:8]
            ...
            if h16_payload is None:
                ...
                output = model(
                    kspace, mask, adjacent_kspace=adjacent_kspace
                )
...
def save_model(args, exp_dir, epoch, model, optimizer, best_val_loss, is_new_best):
    ...
    if args.save_every_epoch:
        shutil.copyfile(exp_dir / 'model.pt', exp_dir / f'epoch_{epoch:03d}.pt')
    if is_new_best:
...
def train(args):
    ...
    if resume is not None:
        ...
        print(f'Resuming at epoch index {start_epoch}')
    else:
        best_val_loss = 1.
        start_epoch = 0
