"""PromptMR+ building blocks.

Adapted from https://github.com/hellopipu/PromptMR-plus under the Rutgers
Non-commercial Research License retained in this directory.
"""
...
def batched_mask_center(
    ...
    """Official fastMRI helper, kept local to avoid package-path coupling."""
    if mask_from.shape != mask_to.shape or mask_from.ndim != 1:
        raise ValueError("mask_from and mask_to must be matching 1-D tensors")
    if mask_from.shape[0] == 1:
        return mask_center(x, int(mask_from), int(mask_to))
    if x.shape[0] != mask_from.shape[0]:
        raise ValueError("mask bounds must have batch_size length")
    masked = torch.zeros_like(x)
    for index, (start, end) in enumerate(zip(mask_from, mask_to)):
        masked[index, :, :, start:end] = x[index, :, :, start:end]
    return masked
